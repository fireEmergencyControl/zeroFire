...
class MqttWorker:
    ...
    def on_message(self,client, userdata, message): 
        try:
            ...
            # elif myval == "180":
            #     self.servo.getDuty(int(myval))
            if mytopic[1] == "forward":
                self.motor.forward()
            elif mytopic[1] == "backward":
                self.motor.backward()
            elif mytopic[1] == "leftforward":
                self.motor.left()
            elif mytopic[1] == "leftbackward":
                self.motor.backwardleft()
            elif mytopic[1] == "rightforward":
                self.motor.right()
            elif mytopic[1] == "rightbackward":
                self.motor.backwardright()
            elif mytopic[1] == "stop":
                self.motor.stop()    
            elif mytopic[0] == "servo":
                self.servo.getDuty(int(myval))
            elif mytopic[1] == "watermotor_on":
                self.watermotor.watermotor_on()
            elif mytopic[1] == "watermotor_off":
                self.watermotor.watermotor_off()
            elif mytopic[1] == "start":
                print("================================")
                while True:
