package com.multicamp.mainapp.zerofiretab.tabLocation.controldata
...
class MyMqtt(context: Context, uri:String) {
    ...
    // 메세지가 전송되면 callback메소드가 호출될 수 있도록 등록 ---------------------------------------  3)
    // 액티비티에서 mysetCallback메소드를 호출하면서 실핼할 메소드를 넘기면 MqttAndroidClient객체의
    // setCallback메소드가 호출되며 콜백메소드가 등록된다.
    ...
    fun connect(topic:Array<String>){
        //연결하기 위해서 필요한 여러 가지 정보를 담고 있는 객체
        ...
        mqttClient.connect(mqttConnectOptions,null,
                            object:IMqttActionListener{
                                override fun onSuccess(asyncActionToken: IMqttToken?) {
                                    ...
                                    //topic을 여러 개 등록할 수 있으므로 배열처리
                                    //모든 전달된 토픽을 subscribe
                                    //for문이나 if문으로 처리할 수 있지만 map함수를 이용하면 편하게 작업가능
                                    topic.map{
                                        subscribeTopic(it)
                                    }
                                }
    ...
    fun publish(topic:String,payload:String,qos:Int=0){
        ...
        //메시지객체에 payload와 메시지전송품질을 설정
        //네트워크로 전송되므로 byte로 변경
        message.payload = payload.toByteArray()
        message.qos = qos
        //메시지전송하기 (publish) - publish가 성공/실패하는 경우 이벤트가 발생하기 때문에 리스너등록
